...
class packerMod:
    ...
    def use_esxi_config(self):
        for builder in self.local_packer['builders']:
            ...
            for provisioner in self.local_packer['provisioners']:
                if "script/minimize.sh" in provisioner['scripts']:
                    index = provisioner['scripts'].index("script/minimize.sh")
                    provisioner['scripts'].remove("script/minimize.sh")
                    provisioner['scripts'].insert(index, "{{user `minimize_script`}}")
